...
def get_text_from_file_name_with_encoding(file_name):
    ...
    dic={}
    result = chardet.detect(data)#{'encoding': 'ISO-8859-5', 'confidence': 0.99, 'language': 'Russian'}
    encoding = result['encoding']#'ISO-8859-5
...
def get_basic_alphabet(alpha2_code):
    ...
    with open(PATH_ALPHABET +f"{alpha2_code}_alphabet.txt","r") as f_out:
        chaine = ''
        for mot in f_out.readlines():
            if mot==str():
                chaine+= mot.strip('\n')
            else:
                valeur = mot.encode('latin1').decode('utf-8')
                chaine+= valeur.strip('\n')
    return chaine
print(get_basic_alphabet('fr'))
...
def get_diacriticals(alpha2_code):
    ...
    with open(PATH_ALPHABET +f"{alpha2_code}_diacriticals.txt","r") as f_out:
        dic = {}
        for line in f_out.readlines():
            liste = line.split(' ')
            valeur = liste[1].encode('latin1').decode('utf-8')
            valeur_sans = valeur.split('|')
            res2 = ''.join(valeur_sans).strip('\n')

            dic[liste[0]] = res2
    return dic
print(get_diacriticals('fr'))
