...
class GridCanvas(Grid, tk.Canvas):
    ...
    def __init__(self, master, lines_count, columns_count, cell_size, gutter_size, margin_size, background='yellow', cell_fill='orange', cell_foreground='white', **kw):
        ...
        for cell_number in range(self.get_lines_count() * self.get_columns_count()):
            ...
            self.tag_bind(f'c_{cell_number}', "<Button-1>", lambda event, c=cell_number: self.b1_handler(event, c))
            self.tag_bind(f't_{cell_number}', "<Button-1>", lambda event, c=cell_number: self.b1_handler(event, c))
        master.bind("<space>", self.space_handler)
    ...
    def b1_handler(self, event, cell_number):
