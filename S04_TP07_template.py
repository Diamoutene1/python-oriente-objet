...
class Grid:
    ...
    def fill_random(self, values):
        ...
        for i in range(self.lines_count):
            for j in range(self.columns_count):
                self.grid[i][j] = random.choice(values)
    ...
    def get_diagonal(self):
        ...
        return [self.grid[i][i] for i in range(min(self.lines_count, self.columns_count))]

    def get_anti_diagonal(self):
        """ Extrait l'antidiagonale de la grille."""
        return [self.grid[i][self.columns_count - 1 - i] for i in range(min(self.lines_count, self.columns_count))]
    ...
    def get_grid_str(self, separator='\t'):
        ...
        return '\n'.join(self.get_line_str(i, separator) for i in range(self.lines_count))
    ...
    def get_cell(self, cell_number):
        ...
        line, column = self.get_coordinates_from_cell_number(cell_number)
    ...
    def get_same_value_cell_numbers(self, value):
        ...
        return [self.get_cell_number_from_coordinates(i, j) for i in range(self.lines_count)
                for j in range(self.columns_count) if self.grid[i][j] == value]
    ...
    def get_neighbour(self, line_number, column_number, delta, is_tore=True):
        ...
        delta_line, delta_column = delta
        new_line = line_number + delta_line
        new_column = column_number + delta_column
        if is_tore:
            new_line %= self.lines_count
            new_column %= self.columns_count
        if 0 <= new_line < self.lines_count and 0 <= new_column < self.columns_count:
